...
| `directChannelControl` | ✅ getter | `atvDtv` + `airCable` + `channelNum` | `atvDtv`, `airCable`, `channelNum` | Earlier "not implemented on Frame, Frames have no tuner" was **wrong on both counts**. The getter answered on a 2025 Frame (QN55LS03FAFXZA): `{"airCable":"cable","atvDtv":"tvplus","channelNum":"1001"}`; a set attempt (DTV/AIR/1) returned `-32002`. Note the decompile puts it in the **none-ambient** dispatch map, so it is expected to be unavailable while art mode is on — a `-32601` in art mode must not be read as "unsupported model". |
